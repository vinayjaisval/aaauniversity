...
//                        print_r($comment_er);
                                ...
                                if ($reply->num_rows() > 0):
                                foreach ($reply->result_array() as $key1 => $comment_reply):
                                if ($key1 <= 0){
                                $count = $count + 1;
                                ?>
                                    ...
                                    <div class="comment comment-reply">
                                        <div class="thumbnail">
                                            <?php
                                            $reply_er = $this->user_model->get_all_user($comment_reply['user_id'])->result_array();
                                            ?>
                                            <img src="<?php echo $this->user_model->get_user_image_url($comment_reply['user_id']) ?>"
                                                 alt="Author Images" style="width: 100px"></div>
                                        <div class="comment-content">
                                            <h5 class="title"><?php echo $reply_er[0]['first_name'] . " " . $reply_er[0]['last_name'] ?></h5>
                                            <span class="date"><?php echo date('M d, Y', strtotime($comment_reply['created_at'])) ?></span>
                                            <p>
                                                <?php echo $comment_reply['reply_text'] ?>
                                            </p>
                                            <div class="reply-btn-wrapper">
                                            </div>
                                        </div>
                                ...
                                <?php }else{ ?>
                                    ...
                                    <div class="show_reply">
                                        <div class="comment comment-reply border_top"
                                             id="over_comment<?php echo $comment_reply['id'] ?>">
                                            <div class="thumbnail">
                                                <?php
                                                $reply_er = $this->user_model->get_all_user($comment_reply['user_id'])->result_array();
                                                ?>
                                ...
                                <?php
                                if ($reply->num_rows() == $key1 + 1){ ?>
                                    <p class="comment comment-reply" id="view_all_text" style="cursor: pointer">View
                                        All</p>
                                <?php
                                } ?>
                                    <script type="text/javascript">
                                        $(document).ready(function () {
                                            //$('#over_comment<?php //echo $comment_reply['id']?>//').hide()
                                            $('.show_reply').hide()
                                        });
                                        ...
                                        $('#view_all_text').click(function () {
                                            ...
                                            //$('#over_comment<?php //echo $comment_reply['id']?>//').toggle("medium");
                                            $('.show_reply').toggle("medium");
                                        });
                                ...
                                endforeach;
                                    ...
                                    <!-- start view all Comment Code -->

                                <?php else:
                                $count++ ?>
                                    <div class="show_comment" id="comment_hide<?php echo $blog_comment['id'] ?>"
                                        ...
                                        if ($reply->num_rows() > 0):
                                            foreach ($reply->result_array() as $key1 => $comment_reply):
                                                ...
                                                <?php }else{  $count = $count + 1;?>

                                                    <div class="show_reply1" id="over_comment1<?php echo $comment_reply['id'] ?>">
                                                        <div class="comment comment-reply border_top"
                                                             id="over_comment<?php echo $comment_reply['id'] ?>">
                                                            <div class="thumbnail">
                                                                <?php
                                                ...
                                                <?php
                                                if ($reply->num_rows() == $key1 + 1){ ?>
                                                    <p class="comment comment-reply" id="view_all_text<?php echo $comment_reply['id'] ?>" style="cursor: pointer">View All</p>
                                                <?php
                                                } ?>
                                                    <script type="text/javascript">
                                                        $(document).ready(function () {
                                                            $('#over_comment1<?php echo $comment_reply['id']?>').hide()

                                                            // $('.show_reply').hide()
                                                        });

                                                        $('#view_all_text<?php echo $comment_reply['id'] ?>').click(function () {
                                                            if ($(this).text() === 'View All') {
                                                                $(this).text('Close Reply');
                                                                console.log("<?php echo $count;?>")
                                                            } else if ($(this).text() === 'Close Reply') {
                                                                $(this).text('View All');
                                                                console.log("<?php echo 'close'.$count;?>")
                                                            }

                                                            //$('#over_comment1<?php //echo $comment_reply['id']?>//').toggle("medium");
                                                            $('.show_reply1').toggle("medium");
                                                        });
