                                    ...
                                    <div class="tab-pane" id="basic">
                                        <div class="row justify-content-center">
                                            <div class="col-xl-8">
                                                ...
                                                <div class="form-group row mb-3">
                                                    <label class="col-md-2 col-form-label" for="course_title"><?php echo get_phrase('instructor_name'); ?><span class="required">*</span></label>
                                                    <div class="col-md-10">
                                                        <?php

                                                        $ex= explode(',',$course_details['user_id']);
                                                        ?>
                                                        <input type="text" class="form-control" id="new_instructor" hidden name="new_instructor" placeholder="<?php echo get_phrase('new_instructors'); ?>" value="<?php echo $course_details['user_id']?>">

                                                        <select class="form-control select2" data-toggle="select2" name="" id="course_title" multiple required disabled>
                                                            <option value=""><?php echo get_phrase('select_a_instructor'); ?></option>
                                                            <?php foreach ($get_user->result_array() as $key => $user) : ?>

                                                                <option value="<?php echo $user['id']; ?>" <?php foreach ($ex as $key => $value) : ?> <?php if ($value == $user['id']) echo 'selected'; ?> <?php endforeach; ?>><?php echo $user['first_name'];
                                                                    ?></option>
                                                            <?php endforeach; ?>
