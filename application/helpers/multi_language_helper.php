...
if ( ! function_exists('saveDefaultLevelJSONFile'))
{
    function saveDefaultLevelJSONFile($language_code){
        $language_code = strtolower($language_code);
        if(!file_exists(APPPATH.'level/'.$language_code.'.json')){
            $fp = fopen(APPPATH.'level/'.$language_code.'.json', 'w');
            $newLangFile = APPPATH.'level/'.$language_code.'.json';
            $enLangFile   = APPPATH.'level/english.json';
            copy($enLangFile, $newLangFile);
            fclose($fp);
        }
    }
