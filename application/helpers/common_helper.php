...
if (!function_exists('print_array')) {
    function print_array($array_data)
    {
        echo "<pre>";
        print_r($array_data);
        echo "</pre>";
...
if (!function_exists('date_diff_days')) {
    function date_diff_days($your_date = '')
    {
        // Calculating the difference in timestamps
//        $diff = strtotime($your_date) - strtotime(date('d-m-Y'));
//
//        // 1 day = 24 hours
//        // 24 * 60 * 60 = 86400 seconds
//
//        $date = abs(round($diff / 86400));
//        if ($date > 0) {
//            $date = $date . ' Days';
//        } else {
//            $date = 'Today';
...
//        return $date;
        ...
        $r_date = $diff->format("%R%a days");

        if ($r_date > 1) {
            return " <span class='success' style='color: #34D08C'>" .abs($r_date)." Days Left</span>";
        } elseif ($r_date == 1) {
            return "<span style='color: #34D08C'>".abs($r_date)." Day Left</span> ";
        } elseif ($r_date == 0) {
            return "<span style='color: #34D08C'>Today Left</span>";
        }elseif ($r_date == -1) {
            return "<span style='color: #FE6055FF'>".abs($r_date)." Day Overdue</span>";
        } else {
            return "<span style='color: #FE6055FF'>".abs($r_date)." Days Overdue</span>";
        }
    }
}
