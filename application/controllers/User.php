...
class User extends CI_Controller
{
    ...
    public function course_actions($param1 = "", $param2 = "")
    {
        ...
        if ($param1 == "add") {
            ...
            $r = $this->crud_model->update_course($param2);
...
//                        die();
    ...
    /***************** Chanchal Start Code *******************/
    public function batch_alert()
    {

        $page_data['page_name'] = 'batch_alert';
        $page_data['page_title'] = get_phrase('batch_alert');

        $page_data['students'] = $this->user_model->get_all_user()->result_array();
        $user_id = $_SESSION['user_id'];
        $page_data['batch_data'] = $this->crud_model->get_all_batch_by_instructor($user_id)->result_array();

        $page_data['id'] = $user_id;
        $this->load->view('backend/index', $page_data);
    }

    public function batch_model_form($param1 = '', $param2 = '')
    {
        if ($param1 == 'batch_alert_add') {
            $page_data['page_name'] = 'batch_alert_add';
            $page_data['students'] = $this->user_model->get_all_user()->result_array();
            $page_data['courses'] = $this->crud_model->get_courses()->result_array();
            // print_r($page_data['page_name']);die;
            $page_data['page_title'] = get_phrase('batch_alert_add');

            $this->load->view('backend/index', $page_data);
        } elseif ($param1 == 'add') {

            $this->crud_model->add_batch_model(true); // PROVIDING TRUE FOR INSTRUCTOR
            redirect(site_url('user/batch_alert'), 'refresh');
        } elseif ($param1 == 'batch_alert_edit') {

            $page_data['page_name'] = 'batch_alert_edit';
            $page_data['page_title'] = get_phrase('batch_alert_edit');
            ...
            $page_data['batch_model_data'] = $this->crud_model->show_batch_model_by_id($param2);
            $page_data['id'] = $param2;
            $page_data['user_id'] = $_SESSION['user_id'];
    ...
    public function attendance()
    {
        ...
        $page_data['attendance_data'] = $this->crud_model->get_all_attendance_by_instructor($user_id);
    ...
    public function attendance_form($param1 = '', $param2 = '')
    {
        ...
        if ($param1 == "attendance_add") {
            $page_data['page_name'] = 'attendance_add';
            $page_data['page_title'] = get_phrase('attendance_add');
            $page_data['id'] = $user_id;
            $page_data['batch_data'] = $this->crud_model->get_all_batch_by_instructor($user_id)->result_array();
            $page_data['courses_data'] = $this->crud_model->get_course_by_user_id($user_id);

            $this->load->view('backend/index', $page_data);
        } elseif ($param1 == "add") {
            $this->crud_model->add_attendance(true); // PROVIDING TRUE FOR INSTRUCTOR
            redirect(site_url('user/attendance'), 'refresh');
        } elseif ($param1 == "edit") {
            ...
            if ($this->input->get('date_range') != "") {
                ...
                //                $page_data['instructor_id'] = $user_id;
            ...
            $page_data['page_name'] = 'attendance';
            $page_data['page_title'] = get_phrase('attendance');
            $page_data['attendance_data'] = $this->crud_model->get_attendance_by_date_range($page_data['timestamp_start'], $page_data['timestamp_end'], $page_data['duration'], $user_id);
    ...
    {
        $user_id = $_SESSION['user_id'];
        $page_data['page_name'] = 'salary_slip';
        $page_data['page_title'] = get_phrase('salary_slip');
        $page_data['id'] = $user_id;
        $page_data['salary_slip_data'] = $this->crud_model->get_by_instructor_id_salary_slip($user_id);

        $this->load->view('backend/index', $page_data);
    }

    public function salary_form($param = ' ', $param2 = '')
    {
        $id = base64_decode(urldecode($param2));
        $user_id = $_SESSION['user_id'];

        if ($param == "view_slip") {

            $page_data['pdf_data'] = $this->crud_model->get_salary_slip_by_id($id,$user_id);

            $this->load->view('backend/user/slip_pdf.php', $page_data);
            ...
            $this->dompdf->setPaper('A4', 'portrait ');
            $this->dompdf->render();
            $this->dompdf->stream("slip_pdf.php", array("Attachment" => 1));
//            $this->dompdf->stream("slip_pdf.php", array("Attachment" => 1));
        } elseif ($param == "filter_month") {
            $page_data['page_name'] = 'salary_slip';
            $page_data['page_title'] = get_phrase('salary_slip');
            $page_data['id'] = $user_id;
            ...
            //            echo "of";
            //            die();
            $page_data['salary_slip_data'] = $this->crud_model->get_by_month_salary_slip($user_id, $page_data['month']);
    ...
    //    public function itemRequisitionDetailsPdf($id)
    //    {
    ////        $encrypted_id = $id;
    ////        $id = $this->custom->encrypt_decrypt($id, 'decrypt');
    ////        $data = array();
    ////        $data['encrypted_id'] = $encrypted_id;
    ////        $data['req_details'] = $this->Common_model->getDataById($id, "item_requisition_reciept");
    ////        $data['requisitionDetails'] = $this->Common_model->getItemRequisitionIngredients($id);
    ////        // print_r($data['requisitionDetails']);die;
    ////        $this->load->view('inventory/item_requisition_pdf', $data);
    ////        $html = $this->output->get_output();
    ////        $this->load->library('pdf');
    ////        $this->dompdf->loadHtml($html);
    ////        $this->dompdf->setPaper('A4', 'portrait ');
    ////        $this->dompdf->render();
    ////        $this->dompdf->stream("invoice_details.pdf", array("Attachment" => 0));
    //    }
